    ...
    if len(realties) == 0:
        ...
        for k in range(0, len(realties)):
            ...
            try:
                ...
                realties[k] = eval(realties[k]['data-clickstream'])
                # turning 'data-clickstream' dictionary into an actual python dictionary
        ...
        unitTypes_res = ["Apartamento Padrão", "Casa de Condomínio",  # possible residential realties
        ...
        for j in range(0, len(realties)):
            ...
            if realties[j]['unitTypes'][
                0] == "Studio":  # if the realty is a Studio, it will be written in both 'Residencial.csv'
                # and 'Comercial.csv'

                res_w.writerow([realties[j]['listingId'], realties[j]['unitTypes'][0],
                                realties[j]['areas'][0], realties[j]['bedrooms'][0],
                                realties[j]['suites'][0], realties[j]['bathrooms'],
            ...
            elif realties[j]['unitTypes'][0] in unitTypes_res:
                # if the realty type is in the residential realties list,
            ...
            elif realties[j]['unitTypes'][0] in unitTypes_com:
                # if the realty type is in the business realties list,
            ...
            elif realties[j]['unitTypes'][0] in unitTypes_rur:
                # if the realty type is in the countryside realties list,
        ...
        next_page = driver.find_element_by_id('proximaPagina')
